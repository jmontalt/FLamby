...
# CUDA 11.6.
DRIVER_URL = "https://us.download.nvidia.com/tesla/510.108.03/NVIDIA-Linux-x86_64-510.108.03.run"
K80_DRIVER_URL = (
    "https://us.download.nvidia.com/tesla/470.103.01/NVIDIA-Linux-x86_64-470.103.01.run"
)
...
def check_python_version():
    ...
    raise RuntimeError(
        "Unsupported Python version {}. " "Supported versions: 3.6 - 3.10".format(version)
    )
...
def install_driver_runfile(system: System, version: str):
    ...
    if detect_gpu_device() == TESLA_K80_DEVICE_CODE:
        ...
        binary = "NVIDIA-Linux-x86_64-510.108.03.run"
    ...
    while attempt < 3:
        install_run = run(
            "sh {} -s {} {} --no-cc-version-check".format(binary, dkms, no_drm), check=False
        )
